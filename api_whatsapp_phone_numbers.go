...
type WhatsappPhoneNumbersApiUpdateDisplayNameRequest struct {
	ctx context.Context
	ApiService *WhatsappPhoneNumbersApiService
	wabaId string
	phoneNumber string
	whatsappPhoneNameUpdateRequest *WhatsappPhoneNameUpdateRequest
}

func (r WhatsappPhoneNumbersApiUpdateDisplayNameRequest) WhatsappPhoneNameUpdateRequest(whatsappPhoneNameUpdateRequest WhatsappPhoneNameUpdateRequest) WhatsappPhoneNumbersApiUpdateDisplayNameRequest {
	r.whatsappPhoneNameUpdateRequest = &whatsappPhoneNameUpdateRequest
	return r
}

func (r WhatsappPhoneNumbersApiUpdateDisplayNameRequest) Execute() (*WhatsappPhoneNameUpdateResponse, *http.Response, error) {
	return r.ApiService.UpdateDisplayNameExecute(r)
}

/*
UpdateDisplayName Update a phone number display name
...
Updates a WhatsApp business phone number display name.
 ...
 @param wabaId WhatsApp Business Account ID.
 @param phoneNumber Phone number in [E.164](https://en.wikipedia.org/wiki/E.164) format.
 @return WhatsappPhoneNumbersApiUpdateDisplayNameRequest
*/
func (a *WhatsappPhoneNumbersApiService) UpdateDisplayName(ctx context.Context, wabaId string, phoneNumber string) WhatsappPhoneNumbersApiUpdateDisplayNameRequest {
	return WhatsappPhoneNumbersApiUpdateDisplayNameRequest{
		ApiService: a,
		ctx: ctx,
		wabaId: wabaId,
...
func (a *WhatsappPhoneNumbersApiService) UpdateDisplayNameExecute(r WhatsappPhoneNumbersApiUpdateDisplayNameRequest) (*WhatsappPhoneNameUpdateResponse, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPatch
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *WhatsappPhoneNameUpdateResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "WhatsappPhoneNumbersApiService.UpdateDisplayName")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/whatsapp/phoneNumbers/{wabaId}/{phoneNumber}/displayName"
	localVarPath = strings.Replace(localVarPath, "{"+"wabaId"+"}", url.PathEscape(parameterToString(r.wabaId, "")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"phoneNumber"+"}", url.PathEscape(parameterToString(r.phoneNumber, "")), -1)
	...
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.whatsappPhoneNameUpdateRequest == nil {
		return localVarReturnValue, nil, reportError("whatsappPhoneNameUpdateRequest is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.whatsappPhoneNameUpdateRequest
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["api_key"]; ok {
