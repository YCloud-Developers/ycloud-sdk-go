/*
YCloud API

The [YCloud](https://ycloud.com) API is organized around [REST](https://en.wikipedia.org/wiki/Representational_state_transfer). Our API is designed to have predictable, resource-oriented URLs, return [JSON](https://www.json.org) responses, and use standard HTTP response codes and verbs.

API version: v2
Contact: service@ycloud.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package ycloud

import (
	"encoding/json"
)

// WhatsappInboundMessageReaction When a user reacts to messages with an emoji, the message type is set to `reaction`, and this field is included.
type WhatsappInboundMessageReaction struct {
	// Specifies the `wamid` of the message received that contained the reaction.
	MessageId *string `json:"message_id,omitempty"`
	// This field is included when a user reacts to messages with an emoji. Otherwise, it indicates a user removed the emoji.
	Emoji *string `json:"emoji,omitempty"`
}

// NewWhatsappInboundMessageReaction instantiates a new WhatsappInboundMessageReaction object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewWhatsappInboundMessageReaction() *WhatsappInboundMessageReaction {
	this := WhatsappInboundMessageReaction{}
	return &this
}

// NewWhatsappInboundMessageReactionWithDefaults instantiates a new WhatsappInboundMessageReaction object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewWhatsappInboundMessageReactionWithDefaults() *WhatsappInboundMessageReaction {
	this := WhatsappInboundMessageReaction{}
	return &this
}

// GetMessageId returns the MessageId field value if set, zero value otherwise.
func (o *WhatsappInboundMessageReaction) GetMessageId() string {
	if o == nil || o.MessageId == nil {
		var ret string
		return ret
	}
	return *o.MessageId
}

// GetMessageIdOk returns a tuple with the MessageId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WhatsappInboundMessageReaction) GetMessageIdOk() (*string, bool) {
	if o == nil || o.MessageId == nil {
		return nil, false
	}
	return o.MessageId, true
}

// HasMessageId returns a boolean if a field has been set.
func (o *WhatsappInboundMessageReaction) HasMessageId() bool {
	if o != nil && o.MessageId != nil {
		return true
	}

	return false
}

// SetMessageId gets a reference to the given string and assigns it to the MessageId field.
func (o *WhatsappInboundMessageReaction) SetMessageId(v string) {
	o.MessageId = &v
}

// GetEmoji returns the Emoji field value if set, zero value otherwise.
func (o *WhatsappInboundMessageReaction) GetEmoji() string {
	if o == nil || o.Emoji == nil {
		var ret string
		return ret
	}
	return *o.Emoji
}

// GetEmojiOk returns a tuple with the Emoji field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WhatsappInboundMessageReaction) GetEmojiOk() (*string, bool) {
	if o == nil || o.Emoji == nil {
		return nil, false
	}
	return o.Emoji, true
}

// HasEmoji returns a boolean if a field has been set.
func (o *WhatsappInboundMessageReaction) HasEmoji() bool {
	if o != nil && o.Emoji != nil {
		return true
	}

	return false
}

// SetEmoji gets a reference to the given string and assigns it to the Emoji field.
func (o *WhatsappInboundMessageReaction) SetEmoji(v string) {
	o.Emoji = &v
}

func (o WhatsappInboundMessageReaction) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.MessageId != nil {
		toSerialize["message_id"] = o.MessageId
	}
	if o.Emoji != nil {
		toSerialize["emoji"] = o.Emoji
	}
	return json.Marshal(toSerialize)
}

type NullableWhatsappInboundMessageReaction struct {
	value *WhatsappInboundMessageReaction
	isSet bool
}

func (v NullableWhatsappInboundMessageReaction) Get() *WhatsappInboundMessageReaction {
	return v.value
}

func (v *NullableWhatsappInboundMessageReaction) Set(val *WhatsappInboundMessageReaction) {
	v.value = val
	v.isSet = true
}

func (v NullableWhatsappInboundMessageReaction) IsSet() bool {
	return v.isSet
}

func (v *NullableWhatsappInboundMessageReaction) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableWhatsappInboundMessageReaction(val *WhatsappInboundMessageReaction) *NullableWhatsappInboundMessageReaction {
	return &NullableWhatsappInboundMessageReaction{value: val, isSet: true}
}

func (v NullableWhatsappInboundMessageReaction) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableWhatsappInboundMessageReaction) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


